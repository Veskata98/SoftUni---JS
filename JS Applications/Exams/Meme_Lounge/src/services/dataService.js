...
const endpoints = {
    all: '/memes?sortBy=_createdOn%20desc',
    getById: (id) => `/memes/${id}`,
    getMyMemes: (userId) => `/memes?where=_ownerId%3D%22${userId}%22&sortBy=_createdOn%20desc`,

    create: '/memes',
    edit: (id) => `/memes/${id}`,
    del: (id) => `/memes/${id}`,

    search: (query) => `/memes?where=name%20LIKE%20%22${query}%22`
}
...
export const getMyMemes = async (userId) => {
    return await api.get(baseUrl + endpoints.getMyMemes(userId));
};
